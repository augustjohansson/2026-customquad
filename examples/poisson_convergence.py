"""
Example: Poisson convergence test on circle/sphere domain.

Solves the Poisson equation -laplacian(u) = f on a circle (2D) or
sphere (3D) domain using CutFEM with Nitsche's method for boundary
conditions. Shows optimal convergence in L2 and H10 norms.

Usage:
    python poisson_convergence.py --domain circle --degree 1
    python poisson_convergence.py --domain sphere --degree 2
"""
...
# Add demo directory to path for algoim_utils
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "demo"))
import algoim_utils
import customquad


def solve_poisson(domain, N, degree, betaN=10.0, betas=1.0, verbose=False):
    """Solve Poisson problem on a cut domain."""
    ...
    # Exact solution: u = 1 - r^2 (zero on unit circle/sphere)
    if gdim == 2:
        u_ufl = 1 - x[0]**2 - x[1]**2
        f_ufl = 4.0
    else:
    ...
    L_bdry = -inner(g_ufl, dot(n, grad(v))) + inner(betaN / h * g_ufl, v)
    ...
    return h_val, L2_err, H10_err, vol, area


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Poisson convergence test")
    ...
    print(f"Poisson convergence on {args.domain} with CG{args.degree}")
    print(f"{'N':>5} {'h':>10} {'L2_err':>12} {'H10_err':>12} "
          f"{'vol_err':>12} {'area_err':>12}")
    print("-" * 70)

    results = []
    for N in mesh_sizes:
        h, L2, H10, vol, area = solve_poisson(
        ...
        print(f"{N:>5} {h:>10.4f} {L2:>12.4e} {H10:>12.4e} "
              f"{vol_err:>12.4e} {area_err:>12.4e}")
